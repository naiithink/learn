...
def view_submissions(request,sec_id,assign_id):
    ...
    json_full_scores = json.dumps({std.username:submission.assignment.task.textblanks
        for (std,submission) in recent_submissions if submission})
    ...
    return render(request, 'instr/submissions.html', 
            { 
                ...
                'json_full_scores' : json_full_scores,
            })
...
def zip_submissions(request,sec_id,assign_id):
    ...
    ext = find_extension(assignment.task.language)
    for std,submission in submissions:
        ...
        zip.writestr(zip_info, 
                     codecs.encode(submission.assignment.task.code.dump(submission.answer),
                                   'utf8'))
    response.close()
    return response
...
def grade_submission(request, submission_id):
    ...
    submission.comment = request.POST.get("comment","").strip()
    submission.save()
...
def list_labs(request,sec_id):
    ...
    return render(request, 'instr/labstatus.html', {
            'sec' : sec,
            'show' : show,
        })
...
def report_lab_status(request, labinsec_id):
    ...
    items = [x.number for x in lab.assignment_set.all()]
    ...
    for item in items:
        headers.append('{} passed'.format(item))
        headers.append('{} detailed'.format(item))
        headers.append('{} manual_scores'.format(item))
    writer.writerow(headers)

    for std,stats in all_stats:
        row = [std]
        for passed, detailed, manual in stats:
            row.extend([
                '{:d}'.format(passed),
                '{:.2f}'.format(detailed),
                '{:d}'.format(manual),
...
def lab_status(request,lab_id):
    ...
    # TODO make tasks concrete before computing full manual scores
