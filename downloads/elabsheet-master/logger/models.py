...
class Log(models.Model):
    ...
    seb_keyhash = models.CharField(max_length=200,blank=True,null=True)
    ...
    def create(event,request=None,comment="",user=None,request_ip=None):
        ...
        if request!=None:
            ...
            if getattr(settings, 'LOG_USER_AGENT', False):
                ...
                max_length = Log._meta.get_field('agent').max_length
                agent = agent[:max_length]
            if getattr(settings, 'LOG_SEB_KEYHASH', False):
                seb_keyhash = request.META.get('HTTP_X_SAFEEXAMBROWSER_REQUESTHASH', None)
